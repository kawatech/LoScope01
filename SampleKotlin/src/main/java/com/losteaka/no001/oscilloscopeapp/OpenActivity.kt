...
//    var DataArray = arrayOfNulls<Short>(8000)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
     ...
     //   makeGraph!! { graphProcess() }
        ...
        // 戻るキーでMainActivityに戻る これは実行されない。。。。。うまくいかない。
        /*
...
// これはエラーにならないが、動作はしない。
        ...
        fun onKeyDown(keyCode: Int, event: KeyEvent): Boolean {
            return if (keyCode == KeyEvent.KEYCODE_BACK) {
                // なんらかの処理
                Toast.makeText(applicationContext, "MODORU", Toast.LENGTH_LONG).show()
                true
            } else false
        }
        ------------------------------------------------------------------------------------------ */
...
// *************************** onCreate()ここまで *************************************
    ...
    // 戻るキーの確認のため、メッセージを出してみる。MainActivityではは動作した。ここでは動作しない。
    /* --------------------------------------------------------------------------------------------
...
/* +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
            ...
            //    backr_btn.isEnabled = false     // BACKボタンでMainActivityに戻りたいが、onClickListnerをどこに置くのか
                ...
                // グラフ描画処理をこの関数に入れて、これを非同期(別スレッド)で実行する
                // この関数名を非同期処理で実行する
                graphProcess()
                ...
                /* -------------------------------------------------------------
                BACKボタンをタッチすると、MainActivityの画面に戻る
                グラフ画面を作ってからボタンの処理を書かないと、アプリ停止になる
                ----------------------------------------------------------------- */
...
private fun displayChart() {
    for (row in 0..rowCount - 1) {
        ...
        // ここでは、グラフの上部に時間を入れる。何も表示しない。時刻が異なるため。
     //   val date = Date()
